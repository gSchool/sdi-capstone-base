...
function Approver() {
  ...
  const pendingBadge = async (requestData) => {
    let count = [];
    let test = await requestData;
    for (let i = 0; i < test.length; i++) {
      if (test[i].status === "Pending") count.push(test[i]);
      setCountState(count.length);
    }
    console.log("length", countState);
  };
